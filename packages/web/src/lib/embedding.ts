 ...
 *
 * HTTP goes through the server proxy (`src/lib/tauri-fetch.ts`) so
 * CORS-unfriendly endpoints work the same as the LLM path.
 *
 * NOTE: Vector operations (LanceDB) are currently stubbed in the web
 * version — they require the Rust backend to be implemented. The
 * embedding HTTP fetch still works for generating embeddings.
 */
...
// ── LanceDB v2 operations — STUBBED for web version ──────────────────────
//
// These operations require the Rust LanceDB backend which is not yet
// implemented in the web server. For now, they log warnings and return
// empty/zero results. Enable vector search once the server implements
// the /api/vector/* endpoints.
...
async function vectorUpsertChunks(
  _projectPath: string,
  _pageId: string,
  _chunks: ChunkUpsertInput[],
): Promise<void> {
  console.warn("[Embedding] vectorUpsertChunks: stubbed in web version — vector storage not available")
}
...
async function vectorSearchChunks(
  ...
  console.warn("[Embedding] vectorSearchChunks: stubbed in web version — vector search not available")
  return []
}

async function vectorDeletePage(_projectPath: string, _pageId: string): Promise<void> {
  console.warn("[Embedding] vectorDeletePage: stubbed in web version")
}

async function vectorCountChunks(_projectPath: string): Promise<number> {
  console.warn("[Embedding] vectorCountChunks: stubbed in web version")
  return 0
}
...
export async function dropLegacyVectorTable(_projectPath: string): Promise<void> {
  console.warn("[Embedding] dropLegacyVectorTable: stubbed in web version")
}
